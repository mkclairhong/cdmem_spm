function recall_createEvents(subjID)
...
if isfile([subjDir, '/events/', subject, 'events.csv'])
    ...
    rec_names = {};
    total_trials = unique([events.trialN]);
    recallOnset = cell(length(total_trials), 27);
    for t = 1:8 % trial loop
        ...
        % scan N
        recallOnset(t, 1: length({rec_event.onset_ttl_start})) = {rec_event(:).onset_ttl_start};
        % recallOnset(t:length({rec_event.onset_ttl_start})) = {rec_event.onset_ttl_start};
    ...
    % save to csv files
    save([subjDir, '/events/recall/recallOnset.mat'], 'recallOnset');
    disp(['Successfully saved stimOnset as ', [subjDir, '/events/recall/recallOnset.mat']])
    
    save([subjDir, '/events/recall/recall_cond.mat'], 'cond');
    disp(['Successfully saved cond as ', [subjDir, '/events/recall/recall_cond.mat']])
    
    duration = zeros(8,27); 
    duration = num2cell(duration);
    save([subjDir, '/events/recall/recall_duration.mat'], 'duration');
    disp(['Successfully saved duration as ', [subjDir, '/events/recall/recall_duration.mat']])
else
